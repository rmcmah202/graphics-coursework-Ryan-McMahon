...
void MyShader::init(const std::string& vertFile, const std::string& fragFile)
{
...
void MyShader::Update(const Transform& transform, const TheCamera& camera)
{
...
GLuint MyShader::CreateShader(const std::string& text, unsigned int type)
{
	...
	CheckShaderError(MyShader, GL_COMPILE_STATUS, false, "Error compiling shader!"); //check for compile error
...
void MyShader::CheckShaderError(GLuint MyShader, GLuint flag, bool isProgram, const std::string& errorMessage)
{
	GLint success = 0;
	GLchar error[1024] = { 0 };

	if (isProgram)
		glGetProgramiv(MyShader, flag, &success);
	else
		glGetShaderiv(MyShader, flag, &success);

	if (success == GL_FALSE)
	{
		if (isProgram)
			glGetProgramInfoLog(MyShader, sizeof(error), NULL, error);
		else
			glGetShaderInfoLog(MyShader, sizeof(error), NULL, error);
